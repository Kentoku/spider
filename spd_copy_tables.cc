...
int spider_udf_set_copy_tables_param_default(
  ...
  if (!copy_tables->database)
  {
    ...
    copy_tables->database_length = copy_tables->trx->thd->SPIDER_db_length;
    if (
      !(copy_tables->database = spider_create_string(
        copy_tables->trx->thd->SPIDER_db_str,
...
long long spider_copy_tables_body(
  ...
  table_list->SPIDER_db_str = copy_tables->spider_db_name;
  table_list->SPIDER_db_length = copy_tables->spider_db_name_length;
  table_list->SPIDER_alias_str = table_list->SPIDER_table_name_str =
    copy_tables->spider_real_table_name;
  table_list->SPIDER_table_name_length =
    copy_tables->spider_real_table_name_length;
#ifdef SPIDER_use_LEX_CSTRING_for_database_tablename_alias
  table_list->SPIDER_alias_length = table_list->SPIDER_table_name_length;
#endif
  table_list->lock_type = TL_READ;

  DBUG_PRINT("info",("spider db=%s", table_list->SPIDER_db_str));
  DBUG_PRINT("info",("spider db_length=%zd", table_list->SPIDER_db_length));
  DBUG_PRINT("info",("spider table_name=%s",
    table_list->SPIDER_table_name_str));
